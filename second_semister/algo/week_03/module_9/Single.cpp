...
int main()
{
    ...
    // cout << level[source.first][source.second] << endl;
    // return 0;
    int l = 0;
    while (1)
    {
        pair<int, int> new_source = source;
        for (int i = 0; i < 4; i++)
        {
            int new_x = new_source.first + dx[i];
            int new_y = new_source.second + dy[i];
            if (level[new_x][new_y] == l)
            {
                if (i == 0)
                {
                    cout << "R";
                    new_source = {new_x, new_y};
                }
                else if (i == 1)
                {
                    cout << "L";
                ...
                {
                    cout << "D";
                    new_source.first = new_x;
                    new_source.second = new_y;
                }
                l++;
            }
            if (level[dst.first][dst.second] == l)
            {
                return 0;
            }
        }
